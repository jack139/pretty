...
class handler: 
    def POST(self, version='v1'):
        ...
        param = web.input(app_id='', dev_id='', ver_code='', session='', category='', 
            page_size='', page_index='', sort_by='', sign='')

        if '' in (param.app_id, param.dev_id, param.ver_code, param.page_size, param.page_index, param.sign):
            return json.dumps({'ret' : -2, 'msg' : '参数错误'})

        #验证签名
        md5_str = app_helper.generate_sign([param.app_id, param.dev_id, param.ver_code, param.session,
            param.category, param.page_size, param.page_index, param.sort_by])

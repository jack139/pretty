...
def write_tmp(filename, chunk_num, data): # 写入临时文件目录
    to_path='%s/tmp_%s' % (setting.media_store_path, filename)
...
def write_to_file(filename, filename2):
    tmp_path='%s/tmp_%s' % (setting.media_store_path, filename)
...
class handler: # 
    def POST(self):
        if helper.logged(helper.PRIV_USER|helper.PRIV_MCH):
            ...
            obj_id = user_data.get('obj_id')
            new_image_name = user_data.get('new_image_name')
